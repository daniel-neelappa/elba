...
#Container Setup
if [[ "$CONTAINER_TYPE" == "Docker" ]]; then
  echo "[$(date +%s)] worker Container setup on host:"
  sessions=()
  n_sessions=0
  for host in $WORKER_HOSTS; do
    ...
    ssh -T -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
      ...
      sudo docker cp ~/.ssh/id_rsa worker:/root/.ssh/
      sudo docker cp -a ~/elba worker:/root/
      sudo docker exec --user root worker bash -c $'$commd'
