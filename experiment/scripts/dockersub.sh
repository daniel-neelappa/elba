...
#Container Setup
if [[ "$CONTAINER_TYPE" == "Docker" ]]; then
  echo "[$(date +%s)] Sub Container setup on host:"
  sessions=()
  n_sessions=0
  for host in $SUB_HOSTS; do
    ...
    ssh -T -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
      ...
      sudo docker cp ~/.ssh/id_rsa sub:/root/.ssh/
      sudo docker cp -a ~/elba sub:/root/
      sudo docker exec --user root sub bash -c $'$commd'
