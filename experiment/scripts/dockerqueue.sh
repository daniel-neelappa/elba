...
#Container Setup
if [[ "$CONTAINER_TYPE" == "Docker" ]]; then
  echo "[$(date +%s)] Queue Container setup on host:"
  sessions=()
  n_sessions=0
  for host in $QUEUE_HOSTS; do
    ...
    ssh -T -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
      ...
      sudo docker cp ~/.ssh/id_rsa queue:/root/.ssh/
      sudo docker cp -a ~/elba queue:/root/
      sudo docker exec --user root queue bash -c $'$commd'
